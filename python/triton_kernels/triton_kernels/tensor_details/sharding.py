...
# Triton-side helpers for querying [start, end) shard ranges inside kernels.
# These mirror the host-side range formulas above but are callable from `@triton.jit` kernels.
@triton.jit
def local_range_for_rank_triton(size):
    """Kernel-callable equivalent of LocalSharding._range_for_rank."""
    zero = tl.zeros(size.shape, dtype=size.dtype)
    return zero, size
...
def range_for_rank_triton(
    ...
    if SHARDING_TYPE == TL_SHARDING_LOCAL:
        return local_range_for_rank_triton(size)
    # TL_SHARDING_RANGE
